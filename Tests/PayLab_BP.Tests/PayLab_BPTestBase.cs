...
namespace PayLab_BP.Tests
{
    public abstract class PayLab_BPTestBase : AbpIntegratedTestBase<PayLab_BPTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected PayLab_BPTestBase()
        {
        ...
        protected void UsingDbContext(Action<PayLab_BPDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<PayLab_BPDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<PayLab_BPDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<PayLab_BPDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<PayLab_BPDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<PayLab_BPDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<PayLab_BPDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<PayLab_BPDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<PayLab_BPDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<PayLab_BPDbContext, Task<T>> func)
        {
            T result;
